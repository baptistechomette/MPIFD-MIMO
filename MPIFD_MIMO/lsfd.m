function [rntot, LRtot, URtot, frfmtot] = lsfd(w, frf, fn, xin, useLRUR, useRealResidues, type_frf)
...
%
% LRtot & URtot   matrix with real Lower and Upper Residual terms
%
...
if ~isempty(fn)
    ...
    % [imag(C)] [imag(FRF)]
    %
    % r are calculated using real LR and UR
    %
    % X=[real(r1) imag(r1) ... real(rn) imag(rn) LR UR]^t
    % to obtain rk and rk* in conjugate form

    if ~useRealResidues % using complex residues
        C = zeros(L, 2*nmodes+2) ;
        if strcmp(type_frf, 'disp')
            coeff = ones(L,1) ;
            C(:,2*nmodes+1:2*nmodes+2) = ...
                useLRUR*[1./(s.^2)  ones(L, 1)] ;
        elseif strcmp(type_frf, 'vel')
            coeff = s ;
            C(:,2*nmodes+1:2*nmodes+2) = useLRUR*[1./s s] ;
        elseif strcmp(type_frf, 'acc')
            coeff = s.^2 ;
            C(:,2*nmodes+1:2*nmodes+2) = useLRUR*[ones(L,1) s.^2] ;
        end
        ...
        for ifrf=1:nfrf
            ...
            LRtot(1, ifrf) = X(2*nmodes+1) ;
            URtot(1, ifrf) = X(2*nmodes+2) ;
            frfmtot(:, ifrf) = C(1:L,:)*X + 1j*C(L+1:2*L,:)*X ;
        end

    else % using real residues
        C = zeros(L, nmodes+4) ;
        if strcmp(type_frf, 'disp')
            coeff = ones(L,1) ;
            C(:,nmodes+1:nmodes+2) = ...
        ...
        elseif strcmp(type_frf, 'vel')
            coeff = s ;
            C(:,nmodes+1:nmodes+2) = useLRUR*[1./s s] ;
        elseif strcmp(type_frf, 'acc')
            coeff = s.^2 ;
            C(:,nmodes+1:nmodes+2) = useLRUR*[ones(L,1) s.^2] ;
        end
        ...
        for ifrf=1:nfrf
            ...
            LRtot(1, ifrf) = X(nmodes+1) ;
            URtot(1, ifrf) = X(nmodes+2) ;
            frfmtot(:, ifrf) = C(1:L,:)*X + 1j*C(L+1:2*L,:)*X ;
        end
    end
else
